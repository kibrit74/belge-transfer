...
const WEB_CRYPTO_TIMEOUT_MS = 250;
...
export function pureJsSha256(bytes) {
  ...
  for (let i = 0; i < totalLen; i += 64) {
    ...
    for (let t = 16; t < 64; t++) {
      const s0 =
        (rightRotate(w[t - 15], 7) ^ rightRotate(w[t - 15], 18) ^ (w[t - 15] >>> 3)) >>> 0;
      const s1 =
        (rightRotate(w[t - 2], 17) ^ rightRotate(w[t - 2], 19) ^ (w[t - 2] >>> 10)) >>> 0;
...
export function readFileAsArrayBuffer(file) {
  if (typeof file.arrayBuffer === "function") {
    return file.arrayBuffer().catch(() => readFileWithReader(file));
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      if (reader.result instanceof ArrayBuffer) {
        ...
        reject(new Error("Dosya okuma sonucu geçersiz."));
      }
    };
...
function digestWithTimeout(buffer) {
  let timeoutId;
  const timeout = new Promise((_, reject) => {
    timeoutId = setTimeout(() => reject(new Error("Web Crypto zaman aşımı.")), WEB_CRYPTO_TIMEOUT_MS);
  });

  return Promise.race([globalThis.crypto.subtle.digest("SHA-256", buffer), timeout]).finally(
...
export async function sha256Base64Url(bytes) {
  if (!(bytes instanceof Uint8Array)) {
    throw new TypeError("SHA-256 için Uint8Array gerekir.");
  }

  if (isContextSecure() && globalThis.crypto?.subtle?.digest) {
    try {
      const buffer = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
      const digest = await digestWithTimeout(buffer);
      return toBase64Url(new Uint8Array(digest));
    } catch {
      // Web Crypto başarısız olursa saf JS yedeğine geç
    }
  }

  const digest = pureJsSha256(bytes);
